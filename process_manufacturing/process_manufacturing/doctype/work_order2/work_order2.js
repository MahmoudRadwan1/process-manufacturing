...
frappe.ui.form.on('Work Order2', {
	validate:function(frm){
		...
		var total_factor=0
		for(var x in frm.doc.finished_products){
			total_factor+=flt(frm.doc.finished_products[x].quantity* frm.doc.finished_products[x].cost_factor)
		}
		frm.set_value('total_cost_factor',total_factor)
}
,
	setup: function (frm) {
...
var prompt_for_qty = function (frm, table, title, qty_required, callback) {
	...
	$.each(frm.doc[table] || [], function(i, row) {
		fields.push({
			fieldtype: "Float",
			label: __("{0} - {1}", [row.item, row.item_name,row.qty]),
			fieldname: row.name
	...
	frappe.prompt(
		...
		function(data) {
			...
			frm.doc[table].forEach(function(line) {
				...
				frappe.model.set_value(line.doctype, line.name, "qty", data[line.name]);
			});
