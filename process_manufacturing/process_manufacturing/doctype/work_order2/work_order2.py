...
class WorkOrder2(Document):
	...
	def on_cancel(self):
		stock_entry = frappe.db.sql("""select name from `tabStock Entry`
			where work_order2 = %s and docstatus = 1""", self.name)
		if stock_entry:
	...
	def get_process_details(self):
		#	Set costing_method
		self.costing_method = frappe.db.get_value("Bill Of Material2", self.process_name, "costing_method")
		#	Set Child Tables
		process = frappe.get_doc("Bill Of Material2", self.process_name)
		if process:
	...
	def set_se_items_finish(self, se):
		...
		se_materials = frappe.get_doc("Stock Entry",{"work_order2": self.name, "docstatus": '1'})
		...
		for item in self.finished_products:
			if item.qty > 0:
				qty_of_total_production = float(qty_of_total_production) + item.qty
				if self.costing_method == "Relative Sales Value":
					...
					if sale_value_of_pdt:
						total_sale_value += float(sale_value_of_pdt) * item.qty
					else:
						frappe.throw(_("Selling price not set for item {0}").format(item.item))

		value_scrap = frappe.db.get_value("Bill Of Material2", self.process_name, "value_scrap")
		if value_scrap:
	...
	def set_se_items(self, se, item, s_wh, t_wh, calc_basic_rate=False, qty_of_total_production=None, total_sale_value=None, production_cost=None):
		if item.qty > 0:
			...
			se_item.cost_factor=item.cost_factor
	...
	def make_stock_entry(self, status):
		...
		stock_entry.work_order2 = self.name
		stock_entry.posting_date=self.date
		stock_entry.total_qty_multiplier=self.qty_multiplier
		stock_entry.total_cost_factor=self.total_cost_factor
		if status == "Submitted":
			stock_entry.stock_entry_type = "Material Transfer for Manufacture"
			stock_entry = self.set_se_items_start(stock_entry)
		if status == "In Process":
			stock_entry.stock_entry_type= "Process Manufacture"
...
def validate_material_qty(se_items, po_items):
	...
	for material in po_items:
		...
		if(qty != material.qty):
			...
			).format(material.item, material.item, material.qty))

def manage_se_submit(se, po):
	if po.docstatus == 0:
		frappe.throw(_("Submit the  Work Order2 {0} to make Stock Entries").format(po.name))
...
def manage_se_cancel(se, po):
	if po.status == "In Process":
		...
		frappe.throw("Work order2 status must be In Process or Completed")
	po.flags.ignore_validate_update_after_submit = True
	po.save()
...
def manage_se_changes(doc, method):
	if doc.work_order2:
		po = frappe.get_doc("Work Order2", doc.work_order2)
		if(method=="on_submit"):
			if po.status == "Submitted":
